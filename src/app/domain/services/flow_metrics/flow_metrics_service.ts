...
function calculateSummary(metrics: FlowMetric[]): FlowMetricsSummary {
  ...
  // サイクルタイム計算（マージ済みのみ）
  const cycleTimes = mergedMetrics
    .map(m => m.cycleTimeHours)
    .filter((t): t is number => t !== null);

  // レビュー待ち時間計算
  const waitTimes = metricsWithReview
    .map(m => m.firstReviewWaitHours)
    .filter((t): t is number => t !== null);
  ...
  return {
    ...
    avgFirstReviewWaitHours: waitTimes.length > 0
      ? waitTimes.reduce((a, b) => a + b, 0) / waitTimes.length
      : 0,
    medianFirstReviewWaitHours: calculateMedian(waitTimes),
    p90FirstReviewWaitHours: calculatePercentile(waitTimes, 90),

    avgReviewCount: metrics.length > 0
      ? metrics.reduce((sum, m) => sum + m.reviewCount, 0) / metrics.length
      : 0,
    avgRevisionRounds: metrics.length > 0
      ? metrics.reduce((sum, m) => sum + m.revisionRounds, 0) / metrics.length
      : 0,
    avgUniqueReviewers: metrics.length > 0
      ? metrics.reduce((sum, m) => sum + m.uniqueReviewerCount, 0) / metrics.length
      : 0,

    avgPRSize: metrics.length > 0
      ? metrics.reduce((sum, m) => sum + m.totalChanges, 0) / metrics.length
      : 0,
    sizeDistribution,

    fastFeedbackRatio: metricsWithReview.length > 0
      ? fastFeedback.length / metricsWithReview.length
      : 0,
    quickMergeRatio: mergedMetrics.length > 0
      ? quickMerge.length / mergedMetrics.length
      : 0,
    lowRevisionRatio: metrics.length > 0
      ? lowRevision.length / metrics.length
      : 0,
  };
}
...
function calculateReviewerMetrics(metrics: FlowMetric[]): ReviewerResponseMetrics[] {
  ...
  for (const metric of metrics) {
    ...
    const sortedReviews = [...metric.reviews].sort(
...
function analyzeBottlenecks(metrics: FlowMetric[], summary: FlowMetricsSummary): BottleneckAnalysis[] {
  ...
  if (summary.avgRevisionRounds > 3) {
    const highRevisionPRs = metrics
      .filter(m => m.revisionRounds > 3)
      .map(m => m.prNumber);
  ...
  if (summary.avgPRSize > 500) {
    const largePRs = metrics
      .filter(m => m.totalChanges > 500)
      .map(m => m.prNumber);
...
export const FlowMetricsService = {
  async fetchAll(): Promise<FlowMetricsData> {
