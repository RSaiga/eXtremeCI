...
export class ReviewTimeRepositoryOnJson implements ReviewTimeRepository {
  find = async (owner: string, repo: string): Promise<ReviewTimes> => {
    ...
    const filtered = pull_reqs.data.filter((data: any) => new Date(data.created_at)?.getTime() >= today.getTime())

    const review_times: ReviewTime[] = []
    for (const pr of filtered) {
      const reviews = await this.find_reviews(owner, repo, pr.number)
