...
export class CommiterRepositoryOnJson implements CommiterRepository {
  find = async (owner: string, repo: string): Promise<Commiters> => {
    const results = await this.commit_by_user(owner, repo)
    const commitCountByUser = {}
    results.forEach((v) => {
      const date = new Date(v.commit.author.date).toLocaleDateString()
      const author = v.author?.login ?? v.commit.author.name
  ...
  private commit_by_user(owner: string, repo: string, maxPage = 3) {
    let results = []

    const fetchPage = async (page: number): Promise<any> =>
      octokit
        .request(`GET /repos/{owner}/{repo}/commits?per_page=100&page=${page}`, {
          owner,
          repo,
        })
