...
export class ReadTimeRepositoryOnJson implements ReadTimeRepository {
  find = async (owner: string, repo: string): Promise<ReadTimes> => {
    const today = new Date()
    today.setDate(today.getDate() - 90)
    const pull_reqs = await this.find_pull_req(owner, repo)
    const pull_req = pull_reqs.data.filter(
      (data) =>
        new Date(data.merged_at)?.getTime() >= today.getTime() ||
        new Date(data.closed_at)?.getTime() >= today.getTime(),
    )
    pull_req.sort(
      (a, b) => new Date(a.merged_at || a.closed_at).getTime() - new Date(b.merged_at || b.closed_at).getTime(),
    )
    const read_times = []
    for (const data of pull_req) {
      const commit = await this.find_commit(owner, repo, data)
      ...
      read_times.push(
        new ReadTime(
          data.user.login,
          data.title,
          merged_at.toLocaleDateString(),
          String((merged_at.getTime() - first_committed_at.getTime()) / (60 * 1000)),
        ),
      )
    }
  ...
  private find_commit(owner: string, repo: string, data) {
    return octokit
      .request(`GET /repos/{owner}/{repo}/pulls/${data.number}/commits`, {
        owner,
        repo,
      })
      .catch((reason) => console.log(reason))
