...
export const CommitQualityDashboard: React.FC<CommitQualityDashboardProps> = ({
  ...
  }, [externalMetrics, activeRepo])

  const loadData = async () => {
    setIsLoading(true)
    try {
      const [metricsData, authorData] = await Promise.all([
        analyzeCommitQuality(selectedRepos),
        analyzeAuthorCommitQuality(selectedRepos),
      ])
    ...
    <Box>
      ...
      <Paper
        sx={{
          ...
          mb: 3,
          bgcolor:
            verdict.color === 'success' ? 'success.light' : verdict.color === 'error' ? 'error.light' : 'warning.light',
        }}
      >
          ...
          <Grid xs={12} md={8}>
            ...
            <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
              <Chip
                ...
                color={
                  metrics.smallCommitRatio >= 0.7 ? 'success' : metrics.smallCommitRatio >= 0.5 ? 'warning' : 'error'
                }
              />
              <Chip
                label={`大きいコミット: ${(metrics.largeCommitRatio * 100).toFixed(0)}%`}
                color={
                  metrics.largeCommitRatio <= 0.1 ? 'success' : metrics.largeCommitRatio <= 0.2 ? 'warning' : 'error'
                }
              />
              <Chip
      ...
      <Paper sx={{ p: 2, mb: 3 }}>
              ...
              {COMMIT_SIZE_CATEGORIES.map((cat, idx) => {
                ...
                return (
                  <TableRow key={cat.name} sx={{ bgcolor: cat.color + '20' }}>
                    ...
                    <TableCell align="center">
                      <strong>
                        {cat.min} - {cat.max === Infinity ? '∞' : cat.max} 行
                      </strong>
                    </TableCell>
        ...
        <Alert severity="info" sx={{ mt: 2 }}>
          <strong>目安:</strong> XS + S（50行以下）が <strong>70%以上</strong> なら「刻み型」、 L + XL（151行以上）が{' '}
          <strong>30%以上</strong> なら「まとめ型」と判定
        </Alert>
      </Paper>
      ...
      <Grid container spacing={2} sx={{ mb: 3 }}>
            ...
            <Bar
              data={{
                ...
                datasets: [
                  {
                    label: '割合 (%)',
                    data: [metrics.smallCommitRatio * 100, metrics.largeCommitRatio * 100],
                    backgroundColor: [
                      metrics.smallCommitRatio >= 0.7
                        ? PrimitiveToken.colors.gray[100]
                        : metrics.smallCommitRatio >= 0.5
                          ? PrimitiveToken.colors.gray[70]
                          : PrimitiveToken.colors.red[50],
                      metrics.largeCommitRatio <= 0.1
                        ? PrimitiveToken.colors.gray[100]
                        : metrics.largeCommitRatio <= 0.2
                          ? PrimitiveToken.colors.gray[70]
                          : PrimitiveToken.colors.red[50],
                    ],
                    borderWidth: 2,
        ...
        <Grid xs={12} md={6}>
          <Paper sx={{ p: 2, height: '100%' }}>
            ...
            <Doughnut
              data={{
                labels: metrics.sizeDistribution.map(
                  (d) =>
                    `${d.category.name} (${d.category.min}-${d.category.max === Infinity ? '∞' : d.category.max}行)`,
                ),
                datasets: [
                  {
            ...
            <Card variant="outlined">
              <CardContent>
                ...
                <LinearProgress
                  ...
                  color={
                    metrics.avgLinesPerCommit <= 50 ? 'success' : metrics.avgLinesPerCommit <= 150 ? 'warning' : 'error'
                  }
                  sx={{ mt: 1 }}
                />
                <Typography variant="caption" color="text.secondary">
            ...
            <Card variant="outlined">
              <CardContent>
                ...
                <Typography
                  variant="h4"
                  color={
                    metrics.giantCommitRatio <= 0.05
                      ? 'success.main'
                      : metrics.giantCommitRatio <= 0.1
                        ? 'warning.main'
                        : 'error.main'
                  }
                >
                ...
                <LinearProgress
                  ...
                  color={
                    metrics.giantCommitRatio <= 0.05 ? 'success' : metrics.giantCommitRatio <= 0.1 ? 'warning' : 'error'
                  }
                  sx={{ mt: 1 }}
                />
                <Typography variant="caption" color="text.secondary">
            ...
            <Card variant="outlined">
              <CardContent>
                ...
                <Typography
                  variant="h4"
                  color={
                    metrics.medianLinesPerCommit <= 30
                      ? 'success.main'
                      : metrics.medianLinesPerCommit <= 100
                        ? 'warning.main'
                        : 'error.main'
                  }
                >
                  ...
                  <TableBody>
                    {authorStats.map((stat) => (
                      <TableRow key={stat.author}>
                        ...
                        <TableCell align="center">
                          <Chip label={stat.workStyleLabel} size="small" color={getWorkStyleColor(stat.workStyle)} />
                        </TableCell>
                        <TableCell align="center">
                          <Chip
        ...
        {verdict.style === 'batch' && (
          <>
            ...
            <Box component="ul" sx={{ pl: 2 }}>
              ...
              <li>
                コミット前に <code>git diff</code> で変更内容を確認し、大きすぎたら分割
              </li>
        ...
        {verdict.style === 'mixed' && (
          <>
            ...
            <Box component="ul" sx={{ pl: 2 }}>
              ...
              <li>
                「WIP」コミットは後で <code>git rebase -i</code> で整理
              </li>
            </Box>
          </>
        )}
