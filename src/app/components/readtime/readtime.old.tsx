...
import { RepoSwitcher } from '../repo_switcher'
        ...
        <Box height={100}>
          ...
          <Box sx={{ mb: 3, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
              <RepoSwitcher />
              <Button component={Link} to={route.commitQuality} variant="outlined" className="no-print">
                コミット品質分析
              </Button>
              <Button component={Link} to={route.qualitySustainability} variant="outlined" className="no-print">
                品質と持続性
              </Button>
              <Button component={Link} to={route.flowMetrics} variant="outlined" className="no-print">
                フロー指標
              </Button>
          ...
          <div className="print-section">
            ...
            <Alert severity="info" sx={{ mb: 2 }}>
              ...
              <strong>注意:</strong>{' '}
              長い（数日以上）とフィードバックが遅れ、ばらつきが大きいと見積もり精度が低下します。
                ...
                <Card>
                  <CardContent>
                    ...
                    <Typography variant="h4">
                      {read_time.median() < 24
                        ? `${read_time.median().toFixed(1)}h`
                        : `${(read_time.median() / 24).toFixed(1)}日`}
                    </Typography>
                  </CardContent>
                </Card>
              </Grid>
              <Grid xs={2}>
                <Card>
                  <CardContent>
                    ...
                    <Typography variant="h4">
                      {read_time.avg() < 24
                        ? `${read_time.avg().toFixed(1)}h`
                        : `${(read_time.avg() / 24).toFixed(1)}日`}
                    </Typography>
                  </CardContent>
                </Card>
              </Grid>
                          ...
                          {read_time.statsByAuthor().map((stat) => (
                            <TableRow key={stat.author}>
                              ...
                              <TableCell align="right">
                                {stat.medianHours < 24
                                  ? `${stat.medianHours}h`
                                  : `${(stat.medianHours / 24).toFixed(1)}日`}
                              </TableCell>
                              <TableCell>
                                <Chip
                                  ...
                                  size="small"
                                  color={
                                    read_time.getCategory(stat.medianHours) === 'Fast'
                                      ? 'success'
                                      : read_time.getCategory(stat.medianHours) === 'Normal'
                                        ? 'info'
                                        : read_time.getCategory(stat.medianHours) === 'Slow'
                                          ? 'warning'
                                          : 'error'
                                  }
                                />
          ...
          <div className="print-section print-page-break">
            ...
            <Alert severity="info" sx={{ mb: 2 }}>
              <strong>測定内容:</strong> PRの変更行数をカテゴリ分け（XS:~10行, S:~50行, M:~250行, L:~500行,
              XL:500行~）。
          ...
          <div className="print-section print-page-break">
            ...
            <Alert severity="info" sx={{ mb: 2 }}>
              ...
              <strong>注意:</strong>{' '}
              24時間以上だとコンテキストスイッチ増加、WIP増加でフロー効率低下。特定の人に集中するとボトルネック化。
          ...
          <div className="print-section print-page-break">
            ...
            <Alert severity="info" sx={{ mb: 2 }}>
              ...
              <strong>注意:</strong>{' '}
              7日以上オープンのPRが多いとWIP過多（リトルの法則でリードタイム悪化）、コンフリクトリスク蓄積。
          ...
          <div className="print-section print-page-break">
            ...
            <Alert severity="info" sx={{ mb: 2 }}>
              ...
              <strong>注意:</strong>{' '}
              特定ペアに偏ると知識がサイロ化（属人化）、バス係数低下。特定の人に負荷集中でバーンアウトリスク。
          ...
          <div className="print-section print-page-break">
            ...
            <Alert severity="info" sx={{ mb: 2 }}>
              ...
              <strong>注意:</strong>{' '}
              上位3人シェアが80%超だとヒーロー依存でバス係数極低。スプリント終盤に活動集中は駆け込み開発の兆候。
