...
export const PrSizeGraph: React.FC<PrSizeGraphProps> = (props) => {
  ...
  const values = [categoryCount.XS, categoryCount.S, categoryCount.M, categoryCount.L, categoryCount.XL]
            ...
            <TableBody>
              {sortedPrSizes.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row, index) => (
                <TableRow key={index} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                  ...
                  <TableCell>
                    <Chip label={row.sizeCategory} color={getCategoryColor(row.sizeCategory)} size="small" />
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
